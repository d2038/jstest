                ...
                error: function (xmlHttpRequest, textStatus, errorThrown) {
                     if(xmlHttpRequest.readyState == 0 || xmlHttpRequest.status == 0) {
                         ...
                         if( xmlHttpRequest.status == '404'){
                            $.post( "/ajax.php?mode=insert_bad_video_bug", { mode: "insert_bad_video_bug", videoid:videoid, status:  xmlHttpRequest.status}).done(function( data ) { });
                         }
                     }
                }
            });
/*
  ...
  function send_bad_video_f(dif,time){
      try{$.post( "/ajax.php?mode=insert_bad_video_buff", { mode: "insert_bad_video_buff", videoid:videoid, dif:dif,frag:current_frag, time:time}).done(function( data ) { });
        }catch(e){console.log(e.message)}
  }
...
if (typeof p2pml != "undefined" && p2pml.hlsjs.Engine.isSupported() && p2pml.core.HybridLoader.isSupported() && typeof videojs.Html5Hlsjs != "undefined") {
                ...
                hlsjs.on("hlsFragChanged", function (_event, data) {
                    ...
                    if(olvideo_html5_api.currentTime > 90 && false){
                        hlsjs.trigger("hlsBufferFlushing", {
                            startOffset: 0, endOffset: olvideo_html5_api.currentTime - 90 
                        });
                        console.log('Flush old chunks From 0 till '+(olvideo_html5_api.currentTime-90));
                        hlsjs.trigger("hlsBufferFlushing", {
                            startOffset: olvideo_html5_api.currentTime+90, endOffset: olvideo_html5_api.duration 
                        });
    ...
    var store_segments = 100,
    forwardSegmentCount = 200,
    p2pDownloadMaxPriority = 100;
    
    if(typeof navigator.deviceMemory == 'undefined'){
        if(device.mobile()||device.ps()||device.tv()||device.xbox()||device.tablet() || is_touch_device()){
            console.log('p2p 1 set map '+store_segments);
        }else{
            store_segments = 500;
            forwardSegmentCount = 1000;
            p2pDownloadMaxPriority = 500;
            console.log('p2p 2 set map '+store_segments);
        }
    }else{
        if(typeof window.performance.memory != 'undefined' && typeof window.performance.memory.jsHeapSizeLimit  != 'undefined'){
            var mem_lim_segments = Math.round(window.performance.memory.jsHeapSizeLimit/1000/1000/10);
            store_segments = mem_lim_segments;
            forwardSegmentCount = mem_lim_segments*2;
            p2pDownloadMaxPriority = mem_lim_segments;
        }else{
            if(navigator.deviceMemory < 8 || (device.mobile()||device.ps()||device.tv()||device.xbox()||device.tablet() || is_touch_device())){
                console.log('p2p 3 set map '+store_segments);
            }else{
                forwardSegmentCount = 1000;
                store_segments = 500;
                p2pDownloadMaxPriority = 500;
                console.log('p2p 4 set map '+store_segments);
            }
        }
    }
    
    var requiredSegmentsPriority = 5;
    var simultaneousHttpDownloads = 1;
    ...
    engine = new p2pml.hlsjs.Engine({
        ...
        "loader":{
               // xhrSetup: function (xhr,url) {xhr.setRequestHeader('Accept','*/*')},
                ...
                "trackerAnnounce": [
                    "wss://wss.commentsengine.com:8443/"+ws
                ]
        }
    });
                    ...
                    if(segmetInMap && segmetInMap !== undefined){
                        ...
                        if(segmentl){
                            ...
                            if(segment && typeof segment.data !== 'undefined'){
                                ...
                                event.ports[0].postMessage({result: segment});
                            }else{
...
loader.on(p2pml.core.Events.SegmentLoaded, function (segment) {
    p2pdownloadedtotal = p2pdownloadedtotal + segment.data.byteLength;
    console.log("Loading finished "+segment.id+", bytes:", segment.data.byteLength);
});
